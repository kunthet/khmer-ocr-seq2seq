"""
Short Text Optimized On-the-Fly Training Script for Khmer OCR Seq2Seq Model

This training script uses:
1. On-the-fly image generation with RandomWidthDataset (heavily favoring short texts)
2. Short text validation set for consistent evaluation
3. Optimized distribution: ~50% very short (1-5 chars), ~25% short (6-10 chars)
4. Reduced storage requirements and unlimited augmentation variety
"""
import torch
...
def create_data_loaders(
    ...
    validation_dir: str = "data/validation_short_text",
    ...
    # Wrap with RandomWidthDataset optimized for short text generation
    train_dataset = RandomWidthDataset(
        base_dataset=base_train_dataset, 
        min_length=1,
        max_length=100,  # Focus on shorter texts
        alpha=0.005,     # Parameter for exponential decay (less important now due to custom distribution)
        config_manager=config_manager
    )
    logging.info(f"Applied RandomWidthDataset wrapper optimized for short texts (1-100 chars)")
    logging.info(f"Expected distribution: ~50% very short (1-5), ~25% short (6-10), ~25% medium+ (11+)")
    ...
    if validation_path.exists() and (validation_path / "images").exists():
        ...
        val_dataset = SyntheticImageDataset(
            ...
            config_manager=config_manager,
            max_samples=3200  # Ensure exactly 3,200 samples
        )
        ...
        # Wrap with RandomWidthDataset for consistent length control
        val_dataset = RandomWidthDataset(
...
def main():
    ...
    parser.add_argument(
        "--validation-dir", 
        type=str, 
        default="data/validation_short_text",
        help="Directory with short text validation images"
    )
    ...
    parser.add_argument(
        "--config", 
        type=str, 
        default="configs/train_config_short_text.yaml",
    ...
    try:
        ...
        if not validation_path.exists():
            logger.error(f"Short text validation set not found: {validation_path}")
            logger.error("Please generate short text validation set first:")
            logger.error("  python generate_fixed_validation_dataset.py")
            return
        ...
        if not (validation_path / "images").exists() or not (validation_path / "metadata.json").exists():
            ...
            logger.error("Please regenerate short text validation set:")
            logger.error("  python generate_fixed_validation_dataset.py")
            return
        
        logger.info(f"Found short text validation set at: {validation_path}")
        ...
        # Training info
        logger.info("Starting short text optimized on-the-fly training...")
        logger.info("Training Method: On-the-fly image generation with RandomWidthDataset")
        logger.info("Dataset Optimization: Heavy bias toward short texts (1-5 chars: ~50%)")
        logger.info(f"Training Samples: {args.train_samples_per_epoch} per epoch (unlimited variety)")
        logger.info(f"Validation Samples: {len(val_dataloader.dataset)} (short text validation set)")
        logger.info("Target: <=3.0% CER for short texts, <=1.0% CER overall")
