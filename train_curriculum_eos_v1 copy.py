...
def setup_logging(log_dir="logs/curriculum_eos_v1"):
...
class CurriculumDataset:
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(sample, dict):
            image = sample['image']
            targets = sample.get('targets', sample.get('target_sequence', []))
            text = sample.get('text', '')
        elif isinstance(sample, (list, tuple)) and len(sample) >= 2:
            image = sample[0]
            targets = sample[1]
            # Extract text if available (OnTheFlyDataset returns 3-tuple)
            text = sample[2] if len(sample) > 2 else ''
        else:
            # Handle other formats - assume first item is image, second is targets
            try:
                image = sample[0]
                targets = sample[1]
                text = ''
            except (IndexError, TypeError):
                raise ValueError(f"Unexpected sample format: {type(sample)} with value {sample}")
        
        # If we have the original text, truncate it properly by syllables
        if text:
            ...
            try:
                ...
                logger.warning(f"Error encoding truncated text '{truncated_text}': {e}")
            ...
            # Fallback to token-level truncation if text is not available
            # Convert to list if tensor
            if isinstance(targets, torch.Tensor):
                targets = targets.tolist()
            
            # Truncate sequence to curriculum max length
            if len(targets) > self.max_length - 1:  # Reserve space for EOS
                targets = targets[:self.max_length - 1]
            
            # Ensure EOS token at end (critical for curriculum learning)
            if len(targets) == 0 or targets[-1] != self.vocab.EOS_IDX:
                targets.append(self.vocab.EOS_IDX)
...
class CurriculumTrainer:
    ...
    def __init__(self, config_manager):
        ...
        self.curriculum_schedule = [
            ...
            (4, 30, "Phase 4: Extended sequences"),
            (6, 50, "Phase 5: Full sequences")
        ]
        ...
        self.checkpoint_manager = CheckpointManager(
            checkpoint_dir="models/checkpoints/curriculum_eos_v1",
    ...
    def run_curriculum_training(self):
        ...
        checkpoint_info = f"📁 Checkpoints saved to: models/checkpoints/curriculum_eos_v1"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str, 
        default=None,
        help="Path to checkpoint to resume from (e.g., models/checkpoints/curriculum_eos_v1/best_model.pth)"
    )
    parser.add_argument(
        "--auto-resume", 
        action="store_true",
    ...
    parser.add_argument(
        "--log-dir", 
        type=str, 
        default="logs/curriculum_eos_v1",
        help="Directory for training logs"
    )
    parser.add_argument(
        "--checkpoint-dir", 
        type=str, 
        default="models/checkpoints/curriculum_eos_v1",
    ...
    if args.checkpoint_dir != "models/checkpoints/curriculum_eos_v1":
